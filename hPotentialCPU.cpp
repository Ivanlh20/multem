...
// Set Atom type
void cPotentialCPU::SetAtomTypes(sAtomTypesCPU AtomTypesi){
...
void cPotentialCPU::Pot2Da(double &r, double &f, double &df){
	...
	r2 = r*r;
	f = df = 0.0;
	switch (AtomTypes.PotPar){
		case 1:
			// 1: Doyle and Turner parameterization - 4 Gaussians - [0, 4]
			for (i=0; i<4; i++){
				cl = AtomTypes.VR.cl[i]; cnl = AtomTypes.VR.cnl[i];
				f += ft = cl*exp(-cnl*r2);
				df += -2.0*cnl*r*ft;
			}
			break;
		...
		case 5:
			...
			break;
		case 6:
			// 6: Lobato parameterization - 3 Yukawas + 3 Exponentials - [0, 12]
			for (i=0; i<5; i++){
				cl = AtomTypes.VR.cl[i]; cnl = AtomTypes.VR.cnl[i]; icnl = 1.0/cnl; 
...
void cPotentialCPU::Vr(int nAtomTypes, sAtomTypesCPU *AtomTypes, int IntType, int Dim, int nr, double *r, double factor, double *f, double *df){
	int i, j;
	double occ, *ft, *dft; 
