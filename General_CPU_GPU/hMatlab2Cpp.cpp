...
void Matlab2InMulSli(const mxArray *mxInMSTEM, sInMSTEM &InMSTEM){
	...
	InMSTEM.BandwidthLimit = ReadValuemxField<int>(mxInMSTEM, 0, "BandwidthLimit");		// 1: true, 2: false

	InMSTEM.ThicknessTyp = ReadValuemxField<int>(mxInMSTEM, 0, "ThicknessTyp");		// 1: Whole specimen, 2: Throught thickness, 3: Through planes
	mxArray *mxThickness;
	mxThickness = mxGetField(mxInMSTEM, 0, "Thickness");
	InMSTEM.nThickness = mxGetM(mxThickness)*mxGetN(mxThickness);						// Number of thickness
	InMSTEM.Thickness = new double[InMSTEM.nThickness];
	ReadValuemxField(mxInMSTEM, 0, "Thickness", InMSTEM.nThickness, InMSTEM.Thickness);	// Array of thicknesses
	...
	switch (InMSTEM.SimType){
		...
		case 2:		// CBED
			...
			InMSTEM.CBED_Space = ReadValuemxField<int>(mxCBED, 0, "Space");	//
			break;
		case 3:		// HRTEM
			mxArray *mxHRTEM;
