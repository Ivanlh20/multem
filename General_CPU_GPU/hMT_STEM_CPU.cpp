...
void cMT_STEM_CPU::freeMemory()
{
	if(IdCall==0) return;

	delete MT_Detector_CPU; MT_Detector_CPU = 0;
...
void cMT_STEM_CPU::SetInputData(cMT_InMulSli_CPU *MT_InMulSli_CPU_i, cMT_MGP_CPU *MT_MGP_CPU_i, int nThk_i)
{
	freeMemory();
	IdCall++;
	...
	MT_Detector_CPU = new cMT_Detector_CPU;
	MT_Detector_CPU->SetInputData(GP, nDet, DetCir);
