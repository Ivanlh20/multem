...
def run():
    ...
    ] = multem.crystalline_materials.Si001_xtl(na, nb, nc, ncu, rmsd_3d)
    ...
    input_multem.iw_psi = read_psi_0_multem(
    ...
    input_multem.cond_lens_ti_a = (
        1.0  # Height proportion of a normalized Gaussian [0, 1]
    )
    input_multem.cond_lens_ti_sigma = (
        dsf_sigma
    )  # Standard deviation of the defocus spread for the Gaussian component (�)
    input_multem.cond_lens_ti_beta = (
        0.0
    )  # Standard deviation of the defocus spread for the Exponential component (�)
    input_multem.cond_lens_ti_npts = (
        4  # Number of integration points. It will be only used if illumination_model=4
    )
    ...
    input_multem.cond_lens_si_a = (
        1.0  # Height proportion of a normalized Gaussian [0, 1]
    )
    input_multem.cond_lens_si_sigma = (
        ssf_sigma
    )  # Standard deviation of the source spread function for the Gaussian component: For parallel ilumination(�^-1) otherwise (�)
    input_multem.cond_lens_si_beta = (
        0.0
