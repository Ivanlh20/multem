...
void PotentialConv(AtomTypesCPU AtomTypesCPU, int nr, double *rh, int n, double *Vrh){
	...
	double sigma = AtomTypesCPU.sigma;
	...
	cudaMemcpy(cl, AtomTypesCPU.Vr.cl, 6*cSizeofRD, cudaMemcpyHostToDevice);
	cudaMemcpy(cnl, AtomTypesCPU.Vr.cnl, 6*cSizeofRD, cudaMemcpyHostToDevice);
