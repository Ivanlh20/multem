...
namespace mt
{
  ...
  template <typename T>
  void Amorp_Spec<T>::create(Atom_Data<T> &atoms, T d_min, int Z, T rms_3d, T rho, int seed)
  {
    T depth = m_depth;
  ...
  template <typename T>
  int Amorp_Spec<T>::set_init_values(Atom_Data<T> &atoms, T d_min, int Z, 
  ...
  template <typename T>
  T Amorp_Spec<T>::convert_density(int Z, T m_rho)
  {
  ...
  template <typename T>
  T Amorp_Spec<T>::vol() const 
  { 
    return m_l_x*m_l_y*m_l_z; 
  }

  template <typename T>
  T Amorp_Spec<T>::density(int natoms) const
  {
    return natoms/vol();
  }

  template <typename T>
  bool Amorp_Spec<T>::rand_point(Atom_Data<T> &atoms, r3d<T> &r_o)
  {
