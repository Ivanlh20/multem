...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[])
{
	...
	auto rms_3d = mex_get_num<T>(prhs[1]);
	auto occ = mex_get_num<T>(prhs[2]);
	auto region = mex_get_num<dt_int32>(prhs[3]);
	auto bs = mex_get_r_3d<T>(prhs[4]);
	auto d_min = mex_get_num<T>(prhs[5]);
	auto rho = mex_get_num<T>(prhs[6]);
	dt_int32 seed = (nrhs>7)?mex_get_num<dt_int32>(prhs[7]):300183;

	/***************************************************************************************/
	mt::Spec_Lay_Info<T> spec_lay_info(bs, mt::R_3d<T>(), region);
