...
void il_transmission_function(const mxArray *mxB, mt::Output_Multislice_Matlab &output_multislice)
{
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	{
		il_transmission_function<float, mt::e_host>(prhs[0], output_multislice);
	}
	else if(output_multislice.is_double_host())
	{
		il_transmission_function<double, mt::e_host>(prhs[0], output_multislice);
	}
	if(output_multislice.is_float_device())
	{
		il_transmission_function<float, mt::e_device>(prhs[0], output_multislice);
	}
	else if(output_multislice.is_double_device())
	{
		il_transmission_function<double, mt::e_device>(prhs[0], output_multislice);
	}
}