...
#define cPotf 47.877645145863056		// 2*pi^2*a0
#define c2Pi2a0 10.445539456905012 
...
typedef struct sAtoms {
	int Z;			// Atomic number
	int iZ;			// Relative position
	double x;		// x coordinate
	double y;		// y coordinate
	double z;		// z coordinate
	double sigma;	// RMS Isotropic
...
typedef struct sVoCPU{
	...
	sCoefCPU Vr;		// Real Potential coefficients
	sCoefCPU Vi;		// Imaginary Potential coefficients
...
typedef struct sVoGPU{
	double sigma;		// Isotropic standard deviation
	sCoefGPU Vr;		// Real Potential coefficients
	sCoefGPU Vi;		// Imaginary Potential coefficients
...
typedef struct sAtomTypesCPU{
	...
	sCoefCPU feg;	// Electron scattering factor coefficients
	sCoefCPU fxg;	// X-ray scattering factor coefficients
	sCoefCPU Pr;	// Potential coefficients
	sCoefCPU Vr;	// Potential coefficients
	sCoefCPU VR;	// Projected potential coefficients
...
typedef struct sBT{
	...
	dim3 Bonxny;	// Number of blocks for two dimensional optmized grid (nx, ny)
	dim3 Tonxny;	// Number of threads for two dimensional optmized grid (nx, ny)
...
typedef struct sInMSTEM{
	...
	int STEM_GridType;		// 0: Regular, 1: Logarithmic
	int STEM_ns;			// Sampling points
	int STEM_nucx;			// Number of unit cell in x
	int STEM_nucy;			// Number of unit cell in y
	...
	int STEM_nDetCir;		// Number of circular detectors
	sInDetCir *STEM_DetCir;	// Circular detectors

	double CBED_x0;			// x position
	double CBED_y0;			// y position

	int HRTEM_MEffect;			// 1: Partial coherente mode, 2: Transmission cross coefficient
	int HRTEM_STEffect;			// 1: Spatial and temporal, 2: Temporal, 3: Spatial
	int HRTEM_ZeroDefTyp;			// 1: First atom, 2: Middle point, 3: Last atom, 4: Fix Plane
	double HRTEM_ZeroDefPlane;	// Zero defocus plane
	...
	int HCI_MEffect;			// 1: Partial coherente mode, 2: Transmission cross coefficient
	int HCI_STEffect;			// 1: Spatial and temporal, 2: Temporal, 3: Spatial
	int HCI_ZeroDefTyp;			// 1: First atom, 2: Middle point, 3: Last atom, 4: Fix Plane
	double HCI_ZeroDefPlane;	// Zero defocus plane

	int EWRS_ZeroDefTyp;			// 1: First atom, 2: Middle point, 3: Last atom, 4: Fix Plane
	double EWRS_ZeroDefPlane;	// Zero defocus plane

	int EWFS_ZeroDefTyp;			// 1: First atom, 2: Middle point, 3: Last atom, 4: Fix Plane
	double EWFS_ZeroDefPlane;	// Zero defocus plane
} sInMSTEM;

/*******************Radial Schrodinger equation********************/
typedef struct sRadSchr{
