						...
						for(auto iDet =0; iDet<detector.size(); iDet++)
						{
							...
							switch (detector.type)
							{
								case multem::eDT_Circular:
								{
									...
									pval = w_i*multem::sum_square_over_Det(*(this->stream), this->input_multislice->grid, g_inner, g_outer, *psi_z_o);
								}
								break;
								...
								{
									pval = w_i*multem::sum_square_over_Det(*(this->stream), this->input_multislice->grid, detector.fR[iDet], *psi_z_o);
								}
								break;
							}

							int iscan = this->input_multislice->iscan;
							output_multislice.image_tot[ithk].image[iDet][iscan] += pval;
						}
						...
						for(auto iDet =0; iDet<detector.size(); iDet++)
						{
							...
							switch (detector.type)
							{
								case multem::eDT_Circular:
								{
									...
									pval = multem::sum_square_over_Det(*(this->stream), this->input_multislice->grid, g_inner, g_outer, psi_z);
								}
								break;
								case multem::eDT_Radial:
								{
								...
								case multem::eDT_Matrix:
								{
									pval = multem::sum_square_over_Det(*(this->stream), this->input_multislice->grid, detector.fR[iDet], psi_z);
								}
								break;
							}
			...
			void psi(int islice_0, int islice_e, value_type_r w_i, Vector<value_type_c, dev> &trans, TOutput_multislice &output_multislice)
			{
				...
				if(0 <= ithk)
				{
					...
					else
					{
						...
						multem::square(*(this->stream), psi_z, m2psi_z);
						multem::add_scale_to_host(output_multislice.stream, w_i, m2psi_z, output_multislice.m2psi_tot[ithk], &m2psi_zh);
					}
				}
			}
