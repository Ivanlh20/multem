	...
	class Atom_Data
	{
		public:
			...
			void get_statistic(Vector<Atom_Type<T, e_host>, e_host> *atom_type_ptr = nullptr)
			{
				...
				if(bAtomTypes)
				{
					R_int_min = R_int_max = (*atom_type_ptr)[Z[0]-1].coef[0].R_max;
					for(auto iatoms = 0; iatoms < size(); iatoms++)
					{
						R_int_min = min((*atom_type_ptr)[Z[iatoms]-1].coef[0].R_max, R_int_min);
						R_int_max = max((*atom_type_ptr)[Z[iatoms]-1].coef[0].R_max, R_int_max);
					}
				}
