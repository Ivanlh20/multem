...
namespace mt
{
    ...
    using iterator = value_type*;
		...
		iterator begin() const { return real; };
		iterator end() const { return (real + m_size); };
	...
	void assign(rmatrix_c &M_i, TVector &M_o)
	{
	...
	//	{
	//		auto sum_partial = std::reduce(M_i.begin()+range.ixy_0, M_i.begin()+range.ixy_e);
	...
	//		for(auto ixy = range.ixy_0; ixy < range.ixy_e; ixy++)
	//		{
	//			auto z = M_o.template get<Value_type<TVector_o>>(ixy) + w_i*std::norm(M_i[ixy]);
	//			M_o.set(ixy, z);
	//		};
	//	};
	...
	//		for(auto ixy = range.ixy_0; ixy < range.ixy_e; ixy++)
	//		{
	//			auto z1 = m2psi_o.template get<T_r>(ixy) + T_r(w_i)*std::norm(psi_i[ixy]);
