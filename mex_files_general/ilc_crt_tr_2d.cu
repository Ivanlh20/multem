	...
	dt_bool b_fit, pMLD &rM_o, pMLD &rdr_o)
{
	...
	mt::Crt_Tr_2d<T, Dev> crt_tr_2d(&stream, &fft_2d, grid_2d, bg_opt, bg, b_fit);
	crt_tr_2d.set_fft_plan();
	auto dR_o = crt_tr_2d(M1, M2, A, txy, p, sigma_g, radius, bd, 2, M1);
	crt_tr_2d.cleanup();

	thrust::copy(M1.begin(), M1.end(), rM_o.begin());
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto rdr_o = mex_create_pVctr<pMLD>(1, 2, plhs[1]);

	if (system_config.is_float32_cpu())
	{
		run_crt_tr_2d<dt_float32, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, radius, rbd, bg_opt, bg, b_fit, rM_o, rdr_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_crt_tr_2d<dt_float64, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, radius, rbd, bg_opt, bg, b_fit, rM_o, rdr_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_crt_tr_2d<dt_float32, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, radius, rbd, bg_opt, bg, b_fit, rM_o, rdr_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_crt_tr_2d<dt_float64, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, radius, rbd, bg_opt, bg, b_fit, rM_o, rdr_o);
	}
}