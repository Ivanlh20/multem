...
void run_at_tr_2d(mt::System_Config &system_config, pMLD &rM_i, 
T bs_x, T bs_y, pMLD &rA, pMLD &rtxy, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	txy = A*txy;
	mt::AT_2d<T, Dev> at_tr_2d(&stream, grid_2d, bg_opt, bg);
	bg = at_tr_2d(M, A, txy, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	{
		run_at_tr_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_at_tr_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_at_tr_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_at_tr_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
