...
void run_pcf_2d(mt::System_Config &system_config, pMLD &rM1_i, 
	pMLD &rM2_i, T bs_x, T bs_y, pMLD &rA, pMLD &rtxy, T p, T sigma_g, 
	pMLD &rbd, mt::eFil_Sel_Typ bg_opt, T bg, pMLD &rM_o)
{
	...
	mt::Pcf_2d<T, Dev> pcf_2d(&stream, &fft_2d, grid_2d, bg_opt, bg);
	pcf_2d.set_fft_plan();
	pcf_2d(M1, M2, A, txy, p, sigma_g, bd, mx_o);
	pcf_2d.cleanup();
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	{
		run_pcf_2d<dt_float32, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, rbd, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_pcf_2d<dt_float64, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, rbd, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_pcf_2d<dt_float32, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, rbd, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_pcf_2d<dt_float64, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, A, txy, p, sigma_g, rbd, bg_opt, bg, rM_o);
	}
}