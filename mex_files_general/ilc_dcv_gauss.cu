...
void mex_run(mt::System_Config& system_config, dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto psnr = (nrhs>system_config.idx_0+3)?mex_get_num<T>(prhs[system_config.idx_0+3]):T(0.025);
	...
	if (mx_i.is_1d())
	{
		...
		mt::fcn_dcv_fs_gauss_1d(grid, fft, fcn_gauss, psnr, mx_io, &stream);
		fft.cleanup();
	}
	else
	{
		...
		mt::fcn_dcv_fs_gauss_2d(grid, fft, fcn_gauss, psnr, mx_io, &stream);
		fft.cleanup();
	}
