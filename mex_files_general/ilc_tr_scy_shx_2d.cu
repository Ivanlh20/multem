...
void run_tr_scy_shx_2d(mt::System_Config &system_config, pMLD &rM_i, 
T bs_x, T bs_y, T scy, T shx, pMLD &rtxy, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	mt::Tr_Scy_Shx_2d<T, Dev> tr_scy_shx_2d(&stream, grid_2d, bg_opt, bg);
	bg = tr_scy_shx_2d(M, scy, shx, txy, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto shx = mex_get_num<dt_float64>(prhs[idx_0+4]);
	...
	{
		run_tr_scy_shx_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, scy, shx, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_tr_scy_shx_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, scy, shx, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_tr_scy_shx_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, scy, shx, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_tr_scy_shx_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, scy, shx, rtxy, bg_opt, bg, rM_o);
	}
