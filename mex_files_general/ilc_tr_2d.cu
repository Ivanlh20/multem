...
void run_tr_2d(mt::System_Config &system_config, pMLD &rM_i, 
T bs_x, T bs_y, mt::R_2d<T> txy, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	mt::Tr_2d<T, Dev> tr_2d(&stream, grid_2d, bg_opt, bg);
	bg = tr_2d(M, txy, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[])
{
	...
	auto txy = (nrhs > idx_0 + 3)?mex_get_r_2d<dt_float64>(prhs[idx_0 + 3]):mt::R_2d<dt_float64>();
	auto bg_opt = (nrhs > idx_0 + 4)?mex_get_num<mt::eFil_Sel_Typ>(prhs[idx_0 + 4]):mt::efst_min;
	auto bg = (nrhs > idx_0 + 5)?mex_get_num<dt_float64>(prhs[idx_0 + 5]):0.0;
	...
	{
		run_tr_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_tr_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_tr_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_tr_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, txy, bg_opt, bg, rM_o);
	}
