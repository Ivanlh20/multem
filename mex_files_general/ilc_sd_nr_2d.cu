...
void run_sd_nr_2d(mt::System_Config &system_config, pMLD &rM_i, 
	T bs_x, T bs_y, pMLD &rds_x_i, pMLD &rds_y_i, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	mt::Sd_nr_2d<T, Dev> sd_nr_2d(&stream, grid_2d, bg_opt, bg);
	bg = sd_nr_2d(M, ds_x, ds_y, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	{
		run_sd_nr_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, rds_x_i, rds_y_i, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_sd_nr_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, rds_x_i, rds_y_i, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_sd_nr_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, rds_x_i, rds_y_i, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_sd_nr_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, rds_x_i, rds_y_i, bg_opt, bg, rM_o);
	}
