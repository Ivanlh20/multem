...
void mex_run(mt::System_Config& system_config, dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto r_wd = mex_get_num<T>(prhs[system_config.idx_0+2]);
	auto sft = (nrhs>system_config.idx_0+3)?mex_get_bool(prhs[system_config.idx_0+3]):false;
	auto r_c_i = (nrhs>system_config.idx_0+5)?mex_get_r_3d_r_c<T>(prhs[system_config.idx_0+5], bs_i/T(2)):bs_i/T(2);
	...
	if (pmx_o.is_1d())
	{
		...
		const auto r_max = (nrhs>system_config.idx_0+4)?mex_get_num<T>(prhs[system_config.idx_0+4]):bs/T(2);
		const auto r_c = (bs_i.x>bs_i.y)?r_c_i.x:r_c_i.y;
		const auto lr = T(2)*r_max;
		mt::Wd_Hann_1d<T> wd_fcn(r_c, lr, r_wd, r_max);
		mt::Grid_1d<T> grid(bs, shape.shape_size());

		mt::fcn_wd_hann_1d(grid, wd_fcn, sft, mx_o, &stream);
	}
	else
	{
		const auto bs = mt::R_2d<T>(bs_i.x, bs_i.y);
		const auto r_max = (nrhs>system_config.idx_0+4)?mex_get_num<T>(prhs[system_config.idx_0+4]):bs.min()/T(2);
		const auto r_c = mt::R_2d<T>(r_c_i.x, r_c_i.y);
		const auto lr = T(2)*r_max;
		mt::Wd_Hann_2d<T> wd_fcn(r_c, lr, r_wd, r_max);
		mt::Grid_2d<T> grid(bs.x, bs.y, shape[1], shape[0]);

		mt::fcn_wd_hann_2d(grid, wd_fcn, sft, mx_o, &stream);
	}
