...
void mex_run(mt::System_Config& system_config, dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto beta = mex_get_num<T>(prhs[system_config.idx_0+2]);
	auto r_wd = (nrhs>system_config.idx_0+3)?mex_get_num<T>(prhs[system_config.idx_0+3]):beta;
	...
	if (pmx_o.is_1d())
	{
		...
		mt::Wd_Exp_1d<T> wd_fcn(r_c, beta, r_wd, r_max);
		mt::Grid_1d<T> grid(bs, shape.shape_size());

		mt::fcn_wd_exp_1d(grid, wd_fcn, sft, mx_o, &stream);
	}
	else
	{
		...
		mt::Wd_Exp_2d<T> wd_fcn(r_c, beta, r_wd, r_max);
		mt::Grid_2d<T> grid(bs.x, bs.y, shape[1], shape[0]);

		mt::fcn_wd_exp_2d(grid, wd_fcn, sft, mx_o, &stream);
	}
