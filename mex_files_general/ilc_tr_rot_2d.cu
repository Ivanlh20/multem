...
void run_tr_rot_2d(mt::System_Config &system_config, pMLD &rM_i, 
T bs_x, T bs_y, T theta, pMLD &rpc, pMLD &rtxy, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	mt::R_2d<T> pc((rpc.size()>0)?rpc[0]:bs_x/2, (rpc.size()>1)?rpc[1]:bs_y/2);
	mt::R_2d<T> txy(rtxy);

	mt::Tr_Rot_2d<T, Dev> tr_rot_2d(&stream, grid_2d, bg_opt, bg);
	bg = tr_rot_2d(M, theta, pc, txy, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto rpc = (nrhs>idx_0+4)?mex_get_pvctr<pMLD>(prhs[idx_0+4]):pMLD();
	auto rtxy = (nrhs>idx_0+5)?mex_get_pvctr<pMLD>(prhs[idx_0+5]):pMLD();
	...
	{
		run_tr_rot_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, theta, rpc, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_tr_rot_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, theta, rpc, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_tr_rot_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, theta, rpc, rtxy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_tr_rot_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, theta, rpc, rtxy, bg_opt, bg, rM_o);
	}
