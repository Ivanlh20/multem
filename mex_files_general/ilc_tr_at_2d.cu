...
void run_tr_at_2d(mt::System_Config &system_config, pMLD &rM_i, 
T bs_x, T bs_y, pMLD &rA, mt::R_2d<T> txy, mt::eFil_Sel_Typ bg_opt, dt_float64 &bg, pMLD &rM_o)
{
	...
	mt::AT_2d<T, Dev> tr_at_2d(&stream, grid_2d, bg_opt, bg);
	bg = tr_at_2d(M, A, txy, mx_o);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	dt_float64 bs_x = rM_i.cols*mex_get_num<dt_float64>(prhs[idx_0+1]);
	dt_float64 bs_y = rM_i.rows*mex_get_num<dt_float64>(prhs[idx_0+2]);
	auto A = mex_get_pvctr<pMLD>(prhs[idx_0+3]);
	auto txy = mex_get_r_2d<dt_float64>(prhs[idx_0+4]);
	...
	{
		run_tr_at_2d<dt_float32, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_tr_at_2d<dt_float64, mt::edev_cpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_tr_at_2d<dt_float32, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_tr_at_2d<dt_float64, mt::edev_gpu>(system_config, rM_i, bs_x, bs_y, A, txy, bg_opt, bg, rM_o);
	}
