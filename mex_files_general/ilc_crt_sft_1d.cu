...
void run_crt_sft_1d(mt::System_Config &system_config, pMLD &rM1_i, pMLD &rM2_i, 
T bs_x, T p, T sigma_g, pMLD &rbd, pMLD &rM_o, pMLD &rdx_o)
{
	...
	mt::Crt_Sft_1d<T, Dev> crt_sft_1d(&fft_1d, grid_1d);
	crt_sft_1d.set_fft_plan();
	auto x = crt_sft_1d(M1, M2, p, sigma_g, bd, 2);
	crt_sft_1d.cleanup();
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	auto rdx_o = mex_create_pVctr<pMLD>(1, 1, plhs[1]);

	if (system_config.is_float32_cpu())
	{
		run_crt_sft_1d<dt_float32, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, p, sigma_g, rbd, rM_o, rdx_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_crt_sft_1d<dt_float64, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, p, sigma_g, rbd, rM_o, rdx_o);
	}
	else if (system_config.is_float32_gpu())
	{
		run_crt_sft_1d<dt_float32, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, p, sigma_g, rbd, rM_o, rdx_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_crt_sft_1d<dt_float64, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, p, sigma_g, rbd, rM_o, rdx_o);
	}
}