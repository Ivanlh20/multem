...
void run_fd_tr_scy_shx_2d(mt::System_Config &system_config, pMLD &rM1_i, 
...
pMLD &rbd, mt::eFil_Sel_Typ bg_opt, T bg, dt_int32 nit_nm, pMLD &raf_o)
{
	...
	T shx = raf_i[0];
	T scy = raf_i[1];
	mt::R_2d<T> txy = mt::R_2d<T>(raf_i[2], raf_i[3]);
	...
	mt::Fd_Tr_Scy_Shx_2d<T, Dev> fd_tr_scy_shx_2d(&stream, &fft_2d, grid_2d, bg_opt, bg);
	fd_tr_scy_shx_2d.set_fft_plan();
	fd_tr_scy_shx_2d(M1, M2, scy, shx, txy, p, sigma_g, radius, bd, nit_nm);
...
void mexFunction(dt_int32 nlhs, mxArray* plhs[], dt_int32 nrhs, const mxArray* prhs[]) 
{
	...
	/***************************************************************************************/
	dt_int32 rows_o = (raf_i.rows>raf_i.cols)?4:1;
	dt_int32 cols_o = (raf_i.rows>raf_i.cols)?1:4;

	auto raf_o = mex_create_pVctr<pMLD>(rows_o, cols_o, plhs[0]);

	if (system_config.is_float32_cpu())
	{
		run_fd_tr_scy_shx_2d<dt_float32, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, raf_i, p, sigma_g, radius, rbd, bg_opt, bg, nit_nm, raf_o);
	}
	else if (system_config.is_float64_cpu())
	{
		run_fd_tr_scy_shx_2d<dt_float64, mt::edev_cpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, raf_i, p, sigma_g, radius, rbd, bg_opt, bg, nit_nm, raf_o);
	}
	if (system_config.is_float32_gpu())
	{
		run_fd_tr_scy_shx_2d<dt_float32, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, raf_i, p, sigma_g, radius, rbd, bg_opt, bg, nit_nm, raf_o);
	}
	else if (system_config.is_float64_gpu())
	{
		run_fd_tr_scy_shx_2d<dt_float64, mt::edev_gpu>(system_config, rM_1i, rM_2i, bs_x, bs_y, raf_i, p, sigma_g, radius, rbd, bg_opt, bg, nit_nm, raf_o);
	}
}