...
TEM.ThkTyp = 3;             % 1: Whole specimen, 2: Throught thickness, 3: Through planes
...
TEM.Thk = (0.0:0.5:41)*c;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
TEM.STEM.line = 0;
TEM.STEM.ns = 20;
TEM.STEM.x1u = 5*a; TEM.STEM.y1u = 5*b;
TEM.STEM.x2u = 6*a; TEM.STEM.y2u = 6*b;
TEM.STEM.nDet = 6;
...
if(TEM.STEM.line==1)
    for i=1:TEM.STEM.nDet
        ...
        subplot(1, 2, 1);
        hold on;
        plot(STEM(1).DetInt(i).Tot, '-b');
        subplot(1, 2, 2);
        hold on;
        plot(STEM(1).DetInt(i).Coh, '-b');
    end;
else
    for j=1:length(STEM)
        for i=1:TEM.STEM.nDet
            figure(i); %clf;
            imagesc(STEM(j).DetInt(i).Tot);
            colormap gray;
            axis image;      
        end;
        sum(STEM(j).DetInt(4).Tot(:))
    end;
end;
