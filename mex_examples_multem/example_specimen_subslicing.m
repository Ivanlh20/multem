...
input_multislice.spec_rot_u0 = [0 1 1]; 					% unitary vector			
...
region = 0;
charge = 0;
input_multislice.spec_atoms = [29, 2, 2, 0.0, 0.8, 1.0, charge; 29, 6, 2, 0.0, 0.8, 1.0, charge];
...
axis equal;
axis([-2 input_multislice.spec_lx+2 -5 input_multislice.spec_lz + 5]);
...
for i = 1:nslice0
    hold on;
    plot([-2 input_multislice.spec_lx+2], [Slice0(i, 1) Slice0(i, 1)], '-r', [-2 input_multislice.spec_lx+2], [Slice0(i, 2) Slice0(i, 2)], '-r');    
    axis equal;
    axis([-2 input_multislice.spec_lx+2 -5 input_multislice.spec_lz + 5]);
end